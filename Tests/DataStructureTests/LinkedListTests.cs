...
namespace Tests
{
    public class LinkedListTests
    {
        ...
        private static readonly int[] expectedArray_1491625 = [1, 4, 9, 16, 25];
        private static readonly int[] expectedArray_24 = [2,4];
        private static readonly int[] expectedArray_45 = [4,5];
        private static readonly int[] expectedArray_1234 = [1,2,3,4];
        private static readonly int[] expectedArray_4321 = [4,3,2,1];
        ...
        public void AddFirst_AddLast_ToArray_And_Count_And_Contains_Work() {
            ...
            Assert.Equal(expectedArray_123, arr);
        ...
        public void Reverse_And_ReverseIterator_Work() {
            ...
            list.Reverse();
            Assert.Equal(expectedArray_4321, list.ToArray());

            var back = list.ReverseIterator().Cast<int>().ToArray();
            Assert.Equal(expectedArray_1234, back);
        }
        ...
        public void AddRange_Appends_All_Items() {
            ...
            list.AddRange(["a","b","c"]);
            Assert.Equal(expectedArray_abc, list.ToArray());
        }
        ...
        public void Extensions_Map_Filter_Reduce_ForEach_Partition_Work_As_Expected() {
            ...
            Assert.Equal(expectedArray_1491625, squares);

            var evens = list.Filter(x => x % 2 == 0).Cast<int>().ToArray();
            Assert.Equal(expectedArray_24, evens);
            ...
            Assert.Equal(expectedArray_123, left.ToArray());
            Assert.Equal(expectedArray_45, right.ToArray());
        }
    }
}
