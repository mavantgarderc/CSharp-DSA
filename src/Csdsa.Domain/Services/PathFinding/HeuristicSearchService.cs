...
{
    public AlgorithmResponse<List<Node<T>>> AStar<T>(Graph<T, double> graph, T start, T end)
    {
        // A* logic
        return new AlgorithmResponse<List<Node<T>>>(
            result: new List<Node<T>>(),
            complexity: new ComplexityAnalysis("O((V + E) log V)", "O(V)", "O((V + E) log V)", "O((V + E) log V)", "O((V + E) log V)", "A* implementation", 0, graph.Nodes.Count),
            algorithmName: AlgorithmType.AStar.ToString());
    }
}
